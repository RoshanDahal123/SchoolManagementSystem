...
public interface ISubjectRepository
{
    ...
    Task<List<Subject>> GetAllAsync(bool includeInactive=false,CancellationToken ct = default);
    Task<List<Subject>> GetByIdsAsync(IEnumerable<Guid> ids, CancellationToken ct = default);
    Task<bool> CodeExistsAsync(string code, CancellationToken ct = default);
    
    Task<bool> CodeExistsForOtherAsync(string code, Guid excludeId, CancellationToken ct = default);
    Task<Subject?> GetByCodeIncludingInactiveAsync(string code, CancellationToken ct = default);
